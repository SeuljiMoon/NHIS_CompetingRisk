...
jskm <- function(sfit,
  ...
  if (is.null(cut.landmark)){
    ...
    df_time_low<-subset(df,time<cut.landmark)
    df_time_high<-subset(df,time>=cut.landmark)
    
    p <- p + geom_step(data = df_time_low, size = 0.75) + geom_step(data = df_time_high, size = 0.75) + 
      scale_linetype_manual(name = ystrataname, values=linetype) +
      scale_colour_brewer(name = ystrataname, palette=linecols)
  }
  
  df_cen<-subset(df,n.censor>=1)
  #Add censoring marks to the line:
  if(marks == TRUE)
      p <- p + geom_point(data = df_cen, aes(x = time, y = surv), shape = shape, colour = "black")
